-- Update system_manage_subscription function with new processing minutes per tier
...
BEGIN
  ...
  -- Set included minutes and storage based on tier with NEW VALUES
  IF tier IS NOT NULL THEN
    CASE tier
      WHEN 'starter' THEN
        new_minutes_included := 5;  -- Updated from 20
        new_storage_limit := 100;
      WHEN 'standard' THEN
        new_minutes_included := 15;  -- Updated from 50
        new_storage_limit := 2048; -- 2TB
      WHEN 'advanced' THEN
        new_minutes_included := 80;  -- Updated from 500
