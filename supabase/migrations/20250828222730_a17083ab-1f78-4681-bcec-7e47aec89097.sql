-- Clean up duplicate transcript segments, keeping the most recent one
DELETE FROM transcript_segments 
WHERE id IN (
  SELECT id FROM (
    SELECT id, 
           ROW_NUMBER() OVER (
             PARTITION BY video_id, start_time 
...
-- Now extend transcript_segments table to support Captions with Intention methodology
...
-- Create unique index for video_id + language + start_time to prevent future duplicates
