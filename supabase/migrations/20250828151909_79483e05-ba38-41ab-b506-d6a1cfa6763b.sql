-- Add embedding functionality to videos table
ALTER TABLE public.videos 
ADD COLUMN embed_enabled boolean NOT NULL DEFAULT false,
ADD COLUMN embed_domains text[],
ADD COLUMN embed_token text,
ADD COLUMN embed_settings jsonb DEFAULT '{"autoplay": false, "controls": true, "width": "100%", "height": "auto"}'::jsonb;

-- Create embed analytics table
...
CREATE INDEX idx_embed_analytics_video_id ON public.embed_analytics(video_id);
CREATE INDEX idx_embed_analytics_created_at ON public.embed_analytics(created_at);
CREATE INDEX idx_videos_embed_enabled ON public.videos(embed_enabled) WHERE embed_enabled = true;