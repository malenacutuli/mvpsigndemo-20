-- Fix get_user_storage_usage to use metadata for file size
...
BEGIN
  ...
  -- Calculate storage usage from videos table metadata
  -- Extract file size from metadata->>'size' or metadata->>'fileSize'
  SELECT 
    COALESCE(SUM(
      COALESCE(
