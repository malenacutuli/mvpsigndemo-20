-- ============================================================================
-- FIX: upsert_transcript_segments - Add character_id support
-- ============================================================================
...
BEGIN
  ...
  FOR seg IN SELECT * FROM jsonb_array_elements(p_segments) LOOP
    ...
    SELECT ts.id INTO existing_segment_id
    FROM public.transcript_segments ts
    WHERE ts.transcript_id = v_transcript_id 
    AND ts.idx = (seg->>'idx')::INTEGER;

    IF existing_segment_id IS NOT NULL THEN
      -- UPDATE existing segment (NOW PRESERVES character_id)
      UPDATE public.transcript_segments ts SET
        ...
        character_id = CASE 
          WHEN seg ? 'characterId' THEN (seg->>'characterId')::UUID
          WHEN seg ? 'character_id' THEN (seg->>'character_id')::UUID
          ELSE ts.character_id
        END
      WHERE ts.id = existing_segment_id;
    ELSE
      -- INSERT new segment (NOW INCLUDES character_id)
      INSERT INTO public.transcript_segments (
        ...
        COALESCE((seg->>'characterId')::UUID, (seg->>'character_id')::UUID)
      );
    END IF;
  END LOOP;
  ...
  WHERE ts.transcript_id = v_transcript_id 
  AND ts.idx NOT IN (
    SELECT (seg_elem->>'idx')::INTEGER 
    FROM jsonb_array_elements(p_segments) seg_elem
  );
END;
$function$;